...
namespace FOKE.Pages.RejectedMembersList
{
    public class IndexModel : PagedListBasePageModel
    {
        ...
        public IActionResult OnGetPagedList(int? pn, int? ps, string so, string sc, string gs, string gsc, string nm, string showProject)
        {
            ...
            var response = _membershipFormRepository.GetAllRejectedMembers(Areaid, ProffessionID, WorkPlaceId);
        ...
        public IActionResult OnPostExportData([FromBody] ExportRequestModel data)
        {
            ...
            var empData = _membershipFormRepository.ExportRejectedMemberstoExcel(
                Areaid, 0, 0, data.GlobalSearchColumn, data.GlobalSearch
            );

            var tempFileName = empData.returnData;

            return new JsonResult(new { tFileName = tempFileName, fileName = "RejectedMembersList.xlsx" });
        }
        ...
        public IActionResult OnPostExportPdf([FromBody] ExportRequestModel request)
        {
            var Area = TempData.Peek("PRO_FILTER_AREA");
            var Areaid = GenericUtilities.Convert<long?>(Area);
            var data = _membershipFormRepository.ExportRejectedMemberstoPdf(Areaid, 0, 0, request.GlobalSearchColumn, request.GlobalSearch);
            return new JsonResult(new { tFileName = data.returnData, fileName = "RejectedMembersList.pdf" });
        }
