...
namespace FOKE.Pages.Sponsorship
{
    public class IndexModel : PagedListBasePageModel
    {
        ...
        public IPagedList<SponsorshipViewModel> pagedListData { get; private set; }

        [BindProperty]
        public long? Statusid { get; set; }

        public IndexModel(ISponsorshipRepository sponsorShipRepository, ISharedLocalizer sharedLocalizer, IDropDownRepository dropDownRepository)
        {
        ...
        public IActionResult OnGetPagedList(int? pn, int? ps, string so, string sc, string gs, string gsc, string nm, string showProject)
        {
            ...
            var response = _sponsorShipRepository.GetAllSponsorData(Statusid);
        ...
        public void setPagedListColumns()
        {
            ...
            objList.Add(new PageListFilterColumns { ColumName = "SponsorshipName", ColumnDescription = _sharedLocalizer.Localize("Sponsorship Name").Value });
            objList.Add(new PageListFilterColumns { ColumName = "Email", ColumnDescription = _sharedLocalizer.Localize("Email").Value });
            objList.Add(new PageListFilterColumns { ColumName = "MobileNo", ColumnDescription = _sharedLocalizer.Localize("Contact Number").Value });
            pageListFilterColumns = objList;
        }

        public JsonResult OnPostDeleteSponsor(int? keyid, int? Id)
        {
            var retData = new ResponseEntity<bool>();
            var objModel = new SponsorshipViewModel();
            objModel.SponsorshipId = Convert.ToInt32(keyid);
            objModel.DiffId = Convert.ToInt32(Id);
            retData = _sponsorShipRepository.DeleteSponsor(objModel);
